...
window.addEventListener('load', function () {
    ...
    let list = userArr.map(renderTemplate).join('');
    ...
    ageFilterInput.addEventListener('click', ({target}) => {
        ...
        if (initialState.gender === 'all') {
            filtered = userArr.filter(user => user.age <= initialState.age) // ровняем фильтр по выбранному возрасту
 
            console.table(filtered);
            let list = filtered.map(renderTemplate).join('');
        ...
        filtered = userArr.filter(person => {
            return person.age <= initialState.age && person.gender === initialState.gender
        })
    ...
    genderFilterInputs.forEach(input => {
        input.addEventListener('click', (e) => {
            ...
            filtered = userArr.filter(user => {
                if (user.gender === initialState.gender && (user.age <= initialState.age)) {
                    return user
                }
            })
